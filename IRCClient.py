...
class IRCClient(object):
    ...
    def connect_to_server(self):
        ...
        message = "USER %s %s %s :%s\r\n" % (self.nick, self.hostname, self.servername, self.realname)
    ...
    def listen_for_server_input(self):
        while not self.request_terminate:
            self.server_read_buffer = self.clientSocket.recv(2048).decode()
    ...
    def process_server_input(self):
        if self.server_read_buffer:
            ...
            messages = self.server_read_buffer.split("\r\n")
            self.server_read_buffer = ""
    ...
    def quit(self, quit_message=None):
        ...
        if quit_message is None:     
            quit_message = self.nick + "has quit"
